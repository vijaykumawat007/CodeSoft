...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        self.geomatry("400*400")
        style = Style(theme = "flatly")
        style.configure("Custom.TEntry", foregound="gray")
        ...
        # Bind event to clear placehoder when input feild is cleard
        self.task_input.bind("<FocusIn>", self.clear_placeholder)
        # Bind event to restore placeholder when input field loses focus
        self.task_input.bind("<FocusOut>", self.restore_placehoder)
        ...
        # create Buttond for marking tasks as done or deleting then
        ...
        ttk.Butoon(self, text="view Stats", style="info.TButton",
                   command=self.view_stats).pack(side=tk.BUTTON, pady=10)
    ...
    def view_stats(self):
        ...
        for i in range (total_count):
            if self.task_list.itmeget(i,"fg")=="green":
                done_count += 1
        messagebox.showinfo("Tasks Statistics", f"Total tasks: {total_count}\nCompleted tasks:{done_count}")
    ...
    def delete_task(self):
        task_index = self.task_list.curselction()
        if task_index:
    ...
    def save_tasks(self):
        ...
        for i in range(self.task_list.size()):
            ...
            color =  self.task_list.itemget(i, "fg")
